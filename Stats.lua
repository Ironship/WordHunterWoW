...
local function computeStats(now)
  ...
  Addon.ForEachEffectiveWord(function(_, entry, isDictionary)
    ...
    if WHW_len(top) < 5 or count > (top[WHW_len(top)].encounterCount or 0) then
      local at = WHW_len(top) + 1
      for i = 1, WHW_len(top) do
        if count > (top[i].encounterCount or 0) then at = i break end
      end
      table.insert(top, at, entry)
      if WHW_len(top) > 5 then table.remove(top) end
    end
  end)
  local best = top
  return {
...
function Addon.toggleStats()
  if not statsFrame then
    statsFrame = WHW_CreateFrame("Frame", "WordHunterWoWStats", UIParent, "BackdropTemplate")
    ...
    local close = WHW_CreateFrame("Button", nil, statsFrame, "UIPanelCloseButton")
    ...
    for index, status in ipairs(statuses) do
      ...
      local group = WHW_CreateFrame("Frame", nil, statsFrame)
      group:SetPoint("TOPLEFT", 0, y)
      group:SetSize(340, 26)
      local dot = group:CreateTexture(nil, "ARTWORK")
      WHW_SetColorTexture(dot, COLORS[status][1], COLORS[status][2], COLORS[status][3], 1)
      dot:SetSize(8, 8)
      ...
      WHW_SetColorTexture(barBg, 0.12, 0.16, 0.22, 1)
      barBg:SetPoint("BOTTOMLEFT", 18, 0)
      barBg:SetSize(304, 3)
      local bar = group:CreateTexture(nil, "ARTWORK")
      WHW_SetColorTexture(bar, COLORS[status][1], COLORS[status][2], COLORS[status][3], 0.85)
    ...
    WHW_SetColorTexture(divider1, 0.20, 0.30, 0.43, 0.55)
    ...
    WHW_SetColorTexture(divider2, 0.20, 0.30, 0.43, 0.55)
