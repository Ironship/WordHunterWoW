...
function Addon.CreateSettingsPanel()
  ...
  -- On Retail this frame is parented into the Options canvas. 1.12 has no
  -- AddOns tab to park it in, so here it is a window of its own -- which also
  -- means it needs a parent, an anchor and a way out: a frame with no anchor is
  -- never drawn, and Show() on one succeeds silently.
  local panel = WHW_CreateFrame("Frame", "WordHunterWoWSettingsPanel", UIParent, "BackdropTemplate")
  ...
  local panelClose = WHW_CreateFrame("Button", nil, panel, "UIPanelCloseButton")
  panelClose:SetPoint("TOPRIGHT", -2, -2)
  panelClose:SetScript("OnClick", function() panel:Hide() end)

  local scroll = WHW_CreateFrame("ScrollFrame", "WordHunterWoWSettingsScroll", panel, "UIPanelScrollFrameTemplate")
  scroll:SetPoint("TOPLEFT", 4, -4)
  scroll:SetPoint("BOTTOMRIGHT", -26, 4)

  local box = WHW_CreateFrame("Frame", "WordHunterWoWSettingsContent", scroll)
  box:SetSize(600, 978)
  ...
  local dropdown = WHW_CreateFrame("Frame", "WordHunterWoWBackgroundDropdown", box, "UIDropDownMenuTemplate")
  dropdown:SetPoint("TOPLEFT", 12, -84)
  WHW_DD_SetWidth(dropdown, 220)

  local preview = WHW_CreateFrame("Frame", nil, box, "BackdropTemplate")
  ...
  local slider = WHW_CreateFrame("Slider", "WordHunterWoWOpacitySlider", box, "OptionsSliderTemplate")
  slider:SetPoint("TOPLEFT", 16, -252)
  slider:SetSize(460, 16)
  slider:SetMinMaxValues(0, 1.0)
  slider:SetValueStep(0.05)
  WHW_SetObeyStepOnDrag(slider, true)
  ...
  local scale = WHW_CreateFrame("Slider", "WordHunterWoWTextScaleSlider", box, "OptionsSliderTemplate")
  ...
  WHW_SetObeyStepOnDrag(scale, true)
  ...
  local markDropdown = WHW_CreateFrame("Frame", "WordHunterWoWWordMarkingDropdown", box, "UIDropDownMenuTemplate")
  markDropdown:SetPoint("TOPLEFT", 12, -358)
  WHW_DD_SetWidth(markDropdown, 220)

  local function UpdateMarkText()
    local mark = Addon.WORD_MARKINGS[Addon.GetWordMarking()]
    WHW_DD_SetText(markDropdown, mark and mark.name or "")
  end
  ...
  local function OnMarkClick(self, arg1)
    ...
    WHW_DD_SetText(markDropdown, Addon.WORD_MARKINGS[key].name)
  ...
  local function InitializeMark(self, level)
    ...
    for _, key in ipairs(Addon.WORD_MARKING_ORDER) do
      local info = WHW_DD_CreateInfo()
      info.text = Addon.WORD_MARKINGS[key].name
      info.arg1 = key
      info.value = key
      -- 1.12's dropdown calls info.func with no arguments and copies no arg1
      -- onto the button, so the choice travels in the closure instead.
      local chosen = key
      info.func = function() OnMarkClick(nil, chosen) end
  ...
  local function sizeSlider(name, y, label, get, set)
    ...
    local s = WHW_CreateFrame("Slider", name, box, "OptionsSliderTemplate")
    s:SetPoint("TOPLEFT", 16, y - 20)
    s:SetSize(460, 16)
    s:SetMinMaxValues(Addon.TEXT_SCALE_MIN, Addon.TEXT_SCALE_MAX)
    s:SetValueStep(0.05)
    WHW_SetObeyStepOnDrag(s, true)
    s:SetValue(get())
  ...
  for _, w in ipairs(Addon.SCALED_WINDOWS) do
    local name = "WordHunterWoW" .. string.upper(string.sub(w.key, 1, 1)) .. string.sub(w.key, 2) .. "Slider"
    local getter = Addon["Get" .. string.upper(string.sub(w.key, 1, 1)) .. string.sub(w.key, 2)]
    local setter = Addon["Set" .. string.upper(string.sub(w.key, 1, 1)) .. string.sub(w.key, 2)]
  ...
  local langDropdown = WHW_CreateFrame("Frame", "WordHunterWoWLanguageDropdown", box, "UIDropDownMenuTemplate")
  langDropdown:SetPoint("TOPLEFT", 12, -628)
  WHW_DD_SetWidth(langDropdown, 220)
  ...
  local function InitializeLang(self, level)
    ...
    for _, locale in ipairs(Addon.SUPPORTED_LOCALE_LIST) do
      ...
      info.value = locale
      local chosen = locale
      info.func = function() OnLangClick(nil, chosen) end
  ...
  local integrated = WHW_CreateFrame("CheckButton", "WordHunterWoWIntegratedCheck", box, "UICheckButtonTemplate")
  ...
  local harvest = WHW_CreateFrame("CheckButton", "WordHunterWoWHarvestCheck", box, "UICheckButtonTemplate")
  ...
  WHW_SetWordWrap(harvestNote, true)
  ...
  local function UpdateDropdownText()
    ...
    WHW_DD_SetText(dropdown, style.name)
  end
  ...
  local function Initialize(self, level)
    ...
    for _, key in ipairs(Addon.BACKGROUND_ORDER) do
      ...
      info.value = key
      local chosen = key
      info.func = function() OnClick(nil, chosen) end
  ...
  panel.refresh = function()
    ...
    for key, slider in pairs(panel.windowSliders or {}) do
      local get = Addon["Get" .. string.upper(string.sub(key, 1, 1)) .. string.sub(key, 2)]
