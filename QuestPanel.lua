  ...
  for _, bit in ipairs(enBits) do
    if bit:IsShown() then
      ...
      elseif sentenceIndex and bit.sentenceIndex == sentenceIndex then
        ...
        WHW_SetColorTexture(bit.glow, bg[1], bg[2], bg[3], bg[4])
        bit.glow:Show()
...
local function refreshPanel()
  ...
  if panel.enContent then
    ...
    if entry then
      ...
      if passageText and passageText ~= "" then
        enBlocks[WHW_len(enBlocks) + 1] = { text = passageText }
      else
        if entry.description and entry.description ~= "" then
          enBlocks[WHW_len(enBlocks) + 1] = { text = entry.description }
        end
        if entry.objectives and entry.objectives ~= "" then
          enBlocks[WHW_len(enBlocks) + 1] = { text = entry.objectives }
        end
      end
      ...
      if caveat then
        enBlocks[WHW_len(enBlocks) + 1] = { text = caveat, caveat = true }
    ...
    for _, block in ipairs(enBlocks) do
      if not block.caveat then
        ...
        if piece ~= "" then enPlainParts[WHW_len(enPlainParts) + 1] = piece end
      end
    end
    ...
    for index, block in ipairs(enBlocks) do
      ...
      if not block.caveat then
        for _, si in ipairs(Addon.TokenSentenceIndexes(block.text)) do
          sentenceOfToken[WHW_len(sentenceOfToken) + 1] = sentenceOffset + si
        end
        sentenceOffset = sentenceOffset + WHW_len(Addon.SplitSentences(block.text))
      end
      local tokenNum = 0
      ...
      for _, tokens in ipairs(Addon.TextLines(block.text)) do
        ...
        if WHW_len(tokens) == 0 then ey = ey - TOKEN_STEP * 0.6 end
        for _, token in ipairs(tokens) do
          eused = eused + 1
          tokenNum = tokenNum + 1
          local bit = enBits[eused]
          if not bit then
            bit = WHW_CreateFrame("Frame", nil, panel.enContent)
            bit.text = bit:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
            bit.text:SetPoint("CENTER")
            WHW_SetWordWrap(bit.text, false)
            enBits[eused] = bit
          end
  ...
  for _, tokens in ipairs(Addon.TextLines(lastQuest.text)) do
    ...
    if WHW_len(tokens) == 0 then y = y - TOKEN_STEP * 0.6 end
    for _, token in ipairs(tokens) do
      used = used + 1
      deTokenNum = deTokenNum + 1
      local button = wordButtons[used]
      if not button then
        button = WHW_CreateFrame("Button", nil, panel.content)
        button.text = button:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
        button.text:SetPoint("CENTER")
        WHW_SetWordWrap(button.text, false)
      ...
      if entry then
        ...
        if marking ~= "color" then
          ...
          WHW_SetColorTexture(button.underline, color[1], color[2], color[3], 1)
...
local function trackQuestEncounters(quest)
  ...
  local seen = {}
  local changed = false
  for token in string.gmatch(tostring(quest.text or ""), "%S+") do
...
local function readCurrentQuest(questLogId)
  ...
  -- 1.12's NPC quest frames carry no id, with or without ClassicAPI, and the
  -- log lookups only work from the log. The title is the only handle an offer
  -- frame gives, and the English records are keyed by id and carry the title --
  -- so that is what identifies the quest here.
  if questId == 0 and WHW_QuestIdByTitle then
    questId = WHW_QuestIdByTitle(title) or 0
  end
  local description = GetQuestText and GetQuestText() or ""
  local objectives = GetObjectiveText and GetObjectiveText() or ""
  -- The frame an NPC opens, when something has translated what it draws. The
  -- title deliberately keeps the API's answer: it is the handle the English
  -- records are found by, and those are in English.
  if WHW_OnScreenQuestText then
    local shownDescription, shownObjectives = WHW_OnScreenQuestText("npc")
    description = shownDescription or description
    objectives = shownObjectives or objectives
  end
  ...
  local function normalizeQuestText(t)
    t = string.gsub(string.gsub(tostring(t or ""), "\r\n", "\n"), "\r", "\n")
    t = string.gsub(t, ">[ \t]*\n[ \t]*(%S)", ">\n\n%1")
    return t
  end
  ...
  offer = string.gsub(offer, "\n\n\n+", "\n\n")
  local progressRaw = GetProgressText and GetProgressText() or ""
  local rewardRaw = GetRewardText and GetRewardText() or ""
  if WHW_OnScreenPassage then
    progressRaw = WHW_OnScreenPassage("progress") or progressRaw
    rewardRaw = WHW_OnScreenPassage("reward") or rewardRaw
  end
  local progressText = Addon.trim(normalizeQuestText(progressRaw))
  local rewardText = Addon.trim(normalizeQuestText(rewardRaw))
...
function Addon.createPanel()
  panel = WHW_CreateFrame("Frame", "WordHunterWoWFrame", UIParent, "BackdropTemplate")
  Addon.panel = panel
  ...
  WHW_SetWordWrap(panel.title, false)
  ...
  local close = WHW_CreateFrame("Button", nil, panel, "UIPanelCloseButton")
  ...
  WHW_SetWordWrap(panel.enTitle, false)
  panel.enTitle:SetText(LABELS.englishHeader)

  panel.enScroll = WHW_CreateFrame("ScrollFrame", nil, panel, "UIPanelScrollFrameTemplate")
  panel.enScroll:SetPoint("TOPLEFT", 18, -40)
  panel.enScroll:SetPoint("BOTTOMRIGHT", panel, "BOTTOM", -20, 72)
  panel.enContent = WHW_CreateFrame("Frame", nil, panel.enScroll)
  ...
  for index, status in ipairs(statuses) do
    ...
    WHW_SetColorTexture(dot, color[1], color[2], color[3], 1)
    dot:SetSize(7, 7)
