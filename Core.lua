...
for _, w in ipairs(Addon.SCALED_WINDOWS) do
  local key = w.key
  Addon["Get" .. string.upper(string.sub(key, 1, 1)) .. string.sub(key, 2)] = scaleGetter(key)
  Addon["Set" .. string.upper(string.sub(key, 1, 1)) .. string.sub(key, 2)] = scaleSetter(key, function()
...
function Addon.RegisterDictionaryProvider(locale, providerId, entries)
  ...
  if not Addon.DictionaryProviders[locale][providerId] then
    Addon.DictionaryProviderOrder[locale][WHW_len(Addon.DictionaryProviderOrder[locale]) + 1] = providerId
  end
...
-- How many glossed words the chosen language actually has loaded.
--
-- On this client a dictionary is sixteen megabytes across six files, and if the
-- game declines to read it there is nothing to see: the addon simply colours
-- nothing, which looks exactly like a broken addon. This is the one number that
-- tells the two apart, so /whw lang prints it.
function Addon.DictionaryCount(locale)
  locale = locale or Addon.GetTargetLocale()
  local providers = Addon.DictionaryProviders[locale]
  if type(providers) ~= "table" then return 0 end
  local total = 0
  for _, entries in pairs(providers) do
...
function Addon.ProgressShares(counts)
  ...
  local i = 1
  while leftover > 0 do
    local part = order[math.mod(i - 1, WHW_len(order)) + 1]
...
function Addon.ForEachEffectiveWord(fn)
  ...
  local emitted = (WHW_len(order) > 1) and {} or nil
  for index = WHW_len(order), 1, -1 do
...
function Addon.ApplyBackground(frame, alphaOverride)
  ...
  WHW_SetColorTexture(surface, reading[1], reading[2], reading[3], 1)
...
local function stripMark(value, mark)
  while string.sub(value, 1, WHW_len(mark)) == mark do value = string.sub(value, WHW_len(mark) + 1) end
  while string.sub(value, -WHW_len(mark)) == mark do value = string.sub(value, 1, -WHW_len(mark) - 1) end
  return value
end
...
function Addon.TextLines(text)
  ...
  for line in string.gmatch(body, "([^\n]*)\n") do
    local tokens = {}
    for token in string.gmatch(line, "%S+") do tokens[WHW_len(tokens) + 1] = token end
    lines[WHW_len(lines) + 1] = tokens
  end
  return lines
end
...
function Addon.SplitParagraphs(text)
  ...
  for line in string.gmatch(body, "([^\n]*)\n") do
    if Addon.trim(line) == "" then
      if WHW_len(buf) > 0 then
        paras[WHW_len(paras) + 1] = table.concat(buf, "\n")
        buf = {}
      end
    else
      buf[WHW_len(buf) + 1] = line
    end
  end
  if WHW_len(buf) > 0 then paras[WHW_len(paras) + 1] = table.concat(buf, "\n") end
  return paras
end
...
function Addon.SplitSentences(text)
  ...
  local function finish()
    ...
    out[WHW_len(out) + 1] = string.sub(text, first, last)
    spans[WHW_len(spans) + 1] = { start = first, finish = last }
    first, last = nil, nil
  end
  ...
  for start, token, after in string.gmatch(text, "()(%S+)()") do
    if last and string.find(string.sub(text, last + 1, start - 1), "[\r\n]") then finish() end
    first, last = first or start, after - 1
    local ending = string.gsub(token, '["\'>%)%]]+$', '')
    for _, quote in ipairs({ "“", "”", "’", "»" }) do ending = stripMark(ending, quote) end
    if (string.find(ending, "[.!?]$") or string.sub(ending, -3) == "…")
...
function Addon.TokenSentenceIndexes(text)
  ...
  local si = 1
  for start in string.gmatch(tostring(text or ""), "()%S+") do
    while spans[si] and start > spans[si].finish do si = si + 1 end
    indexes[WHW_len(indexes) + 1] = si
  end
  return indexes
end
...
local function locateSentence(text, globalIndex)
  ...
  for p, para in ipairs(paras) do
    local n = WHW_len(Addon.SplitSentences(para))
    if globalIndex <= seen + n then
      return p, globalIndex - seen
    end
    seen = seen + n
  end
  local last = WHW_len(paras)
  if last == 0 then return 1, 1 end
  return last, math.max(1, WHW_len(Addon.SplitSentences(paras[last])))
end
...
local function matchInPair(deText, enText, word, deIndex)
  ...
  if WHW_len(enSentences) == 0 then return nil, nil end
  deIndex = math.min(WHW_len(deSentences), math.max(1, deIndex or 1))
  if WHW_len(deSentences) == WHW_len(enSentences) then
  ...
  if WHW_len(deSentences) == 1 and WHW_len(enSentences) > 1 then
    local hit = uniqueGlossHit(enSentences, word)
    if hit then return hit, enSentences[hit] end
  end
  local mapped = mapSentenceIndex(deIndex, math.max(1, WHW_len(deSentences)), WHW_len(enSentences))
...
function Addon.MatchEnglishSentence(deText, enText, word, deSentenceIndex)
  ...
  if not deSentenceIndex then
    -- select() is not a 5.0 function; a single assignment already takes
    -- the first of the two values this returns.
    deSentenceIndex = Addon.SentenceContaining(deText, word)
  ...
  local enSentences = Addon.SplitSentences(enText)
  if WHW_len(enSentences) == 0 then return nil, nil end
  deSentenceIndex = math.min(math.max(1, deSentenceIndex), math.max(1, WHW_len(deSentences)))
  if WHW_len(deParas) == WHW_len(enParas) and WHW_len(deParas) > 0 then
    ...
    if localSentence then
      local seen = 0
      for i = 1, pIndex - 1 do
        seen = seen + WHW_len(Addon.SplitSentences(enParas[i]))
      end
  ...
  local mapped = mapSentenceIndex(deSentenceIndex, WHW_len(deSentences), WHW_len(enSentences))
  return mapped, enSentences[mapped]
end
...
local function withinOneEdit(a, b)
  if WHW_len(a) < 6 or WHW_len(b) < 6 then return false end
  if math.abs(WHW_len(a) - WHW_len(b)) > 1 then return false end
  if string.sub(a, 1, 5) ~= string.sub(b, 1, 5) then return false end
  local prefix = 0
  while prefix < WHW_len(a) and prefix < WHW_len(b) and string.byte(a, prefix + 1) == string.byte(b, prefix + 1) do
    prefix = prefix + 1
  end
  local suffix = 0
  while suffix < WHW_len(a) - prefix and suffix < WHW_len(b) - prefix
    and string.byte(a, WHW_len(a) - suffix) == string.byte(b, WHW_len(b) - suffix) do
    suffix = suffix + 1
  end
  return (WHW_len(a) - prefix - suffix) <= 1 and (WHW_len(b) - prefix - suffix) <= 1
end
...
local function glossKeyMatches(enKey, glossKey, tolerant)
  ...
  if WHW_len(glossKey) < 4 or WHW_len(enKey) < 4 then return false end
  -- Only regular English endings, never arbitrary prefixes (head/headmaster).
  if enKey == glossKey .. "s" then return true end
  if string.find(glossKey, "[sxz]$") or string.find(glossKey, "[cs]h$") then
    if enKey == glossKey .. "es" then return true end
  elseif string.find(glossKey, "[^aeiou]y$") and enKey == string.sub(glossKey, 1, -2) .. "ies" then
    return true
  end
...
local function senseWords(sense)
  local words = {}
  for part in string.gmatch(tostring(sense), "%S+") do
    local key = Addon.wordKey(part)
    if key ~= "" then words[WHW_len(words) + 1] = key end
  end
  return words
end
...
local function addSenses(packed, seen, translation)
  ...
  translation = string.gsub(translation, "%b()", "")
  for sense in string.gmatch(translation, "[^;,/]+") do
    sense = string.gsub(Addon.trim(sense), "^to%s+", "")
    local mark = Addon.utf8Lower(sense)
    if sense ~= "" and not seen[mark] then
      seen[mark] = true
      packed[WHW_len(packed) + 1] = sense
    end
  end
end
...
function Addon.MatchEnglishTokenIndexes(enSentence, deWord, occurrence)
  ...
  if WHW_len(tokens) == 0 then return {} end
  ...
  if WHW_len(key) >= 3 then addSenses(packed, seen, deWord) end
  local ranked = {}
  for i, sense in ipairs(packed) do
    ranked[i] = { sense = sense, words = senseWords(sense), i = i }
  end
  table.sort(ranked, function(a, b)
    if WHW_len(a.words) ~= WHW_len(b.words) then return WHW_len(a.words) > WHW_len(b.words) end
    if WHW_len(a.sense) ~= WHW_len(b.sense) then return WHW_len(a.sense) > WHW_len(b.sense) end
    return a.i < b.i
  end)
  local function findHits(words, tolerant)
    if WHW_len(words) == 0 then return {} end
    local hits = {}
    for i = 1, WHW_len(tokens) - WHW_len(words) + 1 do
      local ok = true
      ...
      if ok then hits[WHW_len(hits) + 1] = { start = i, len = WHW_len(words) } end
    end
    return hits
  end
  ...
  if WHW_len(chosen) == 0 then chosen = collect(true) end
...
function Addon.cleanWord(token)
  local word = string.gsub(string.gsub(Addon.trim(token), "^[%p]+", ""), "[%p]+$", "")
...
function Addon.utf8Lower(text)
  ...
  text = string.gsub(string.gsub(text, "ẞ", "ss"), "ß", "ss")
  -- Decimal, not hex. 5.0's lexer reads a numeral as digits, so 0x97 comes back
  -- as the number 0 followed by the name x97 -- a parse error that takes this
  -- whole file with it, and every later file with that, since none of them find
  -- an addon table to attach to. 151 is C3 97, the multiplication sign; 32 is
  -- the distance from a Latin-1 capital to its lowercase.
  text = string.gsub(text, "\195([\128-\158])", function(byte)
    local code = string.byte(byte)
    if code == 151 then return "\195" .. byte end
...
function Addon.wordKey(word)
  local cleaned = string.gsub(string.gsub(Addon.cleanWord(word), "ẞ", "ss"), "ß", "ss")
...
local function encode(value)
  ...
  return (string.gsub(tostring(value or ""), "([^A-Za-z0-9_.~%-])", function(byte)
...
function Addon.rebuildExport()
  ...
  local keys = {}
  for key in pairs(words) do keys[WHW_len(keys) + 1] = key end
  table.sort(keys)
  local rows = {}
  for _, key in ipairs(keys) do
    local item = words[key]
    rows[WHW_len(rows) + 1] = table.concat({
      encode(item.word),
      item.status or "new",
  ...
  if (WordHunterWoWDB.version or 0) < 9 then
    ...
    if type(words) == "table" then
      ...
      for key, entry in pairs(words) do
        local normalized = string.gsub(string.gsub(strlower(tostring(key)), "ẞ", "SS"), "ß", "ss")
        if normalized ~= key then migrations[WHW_len(migrations) + 1] = { key, normalized, entry } end
      end
      ...
      if type(words) == "table" then
        ...
        for key, entry in pairs(words) do
          ...
          if normalized ~= "" and normalized ~= key then
            migrations[WHW_len(migrations) + 1] = { key, normalized, entry }
          end
        end
        for _, migration in ipairs(migrations) do
          local oldKey, newKey, entry = migration[1], migration[2], migration[3]
...
function Addon.HarvestExportPath()
  -- A 1.12 install has no per-flavour folder, and the game directory is
  -- wherever the player unpacked it -- a name Lua cannot see.
  return "<your WoW 1.12 folder>\\WTF\\Account\\<your account>\\SavedVariables\\WordHunterWoW.lua"
end
...
function Addon.SetupEscapeClose(frame)
  ...
  -- The OnKeyDown arm of this belongs to clients that have
  -- SetPropagateKeyboardInput. 1.12 does not: a frame with EnableKeyboard(true)
  -- and a key handler swallows every key it is sent, and there is no way to
  -- hand the rest back. Wiring it up here would take the movement keys away
  -- from the player for as long as a quest was on screen.
  --
  -- Nothing is lost. 1.12's CloseSpecialWindows hides every visible frame named
  -- in UISpecialFrames, so Escape already closes all of these together --
  -- which is exactly what CloseAll did.
end
