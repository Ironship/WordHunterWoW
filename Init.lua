...
SlashCmdList.WORDHUNTERWOW = function(message)
  ...
  elseif string.match(command, "^harvest") then
    local arg = strtrim(string.match(command, "^%S+%s*(.*)$") or "")
  ...
  elseif string.match(command, "^bg%s+") then
    local key = strlower(strtrim(string.match(command, "^bg%s+(.+)$") or ""))
  ...
  elseif string.match(command, "^opacity%s*") or string.match(command, "^alpha%s*") then
    local valStr = strtrim(string.match(command, "^%S+%s*(.*)$") or "")
    if valStr == "" then
  ...
  elseif string.match(command, "^lang") then
    local arg = strtrim(string.match(command, "^%S+%s*(.*)$") or "")
    if arg == "" then
      local cur = Addon.GetTargetLocale()
      print(string.format("|cff66ccffWordHunterWoW:|r Language: %s (%s), %d dictionary words loaded  •  /whw lang <%s>",
        Addon.SUPPORTED_LOCALES[cur] or cur, cur, Addon.DictionaryCount(),
        table.concat(Addon.SUPPORTED_LOCALE_LIST, "|")))
    else
      ...
      if not found then
        local matches = {}
        for _, loc in ipairs(Addon.SUPPORTED_LOCALE_LIST) do
          if string.sub(strlower(loc), 1, WHW_len(norm)) == norm then matches[WHW_len(matches) + 1] = loc end
        end
        -- "es" matches esES and esMX; only a unique prefix is safe to guess.
        if WHW_len(matches) == 1 then found = matches[1] end
      end
