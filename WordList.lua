...
local function refreshWordList()
  ...
  Addon.ForEachEffectiveWord(function(key, entry)
    ...
    if query ~= "" then
      ...
      if not string.find(haystack, query, 1, true) then return end
    end
    items[WHW_len(items) + 1] = { key = key, entry = entry, status = status, sortKey = sortKey }
  end)
  ...
  for index, item in ipairs(items) do
    ...
    if not row then
      row = WHW_CreateFrame("Button", nil, listFrame.content)
      row:SetHeight(22)
      ...
      WHW_SetWordWrap(row.name, false)
      ...
      WHW_SetWordWrap(row.meta, false)
    ...
    WHW_SetColorTexture(row.statusDot, color[1], color[2], color[3], 1)
  ...
  if listFrame.truncated then
    if WHW_len(items) > 200 then
      listFrame.truncated:SetText(string.format("showing 200 of %d — search to narrow", WHW_len(items)))
...
function Addon.toggleWordList()
  if not listFrame then
    listFrame = WHW_CreateFrame("Frame", "WordHunterWoWList", UIParent, "BackdropTemplate")
    ...
    local close = WHW_CreateFrame("Button", nil, listFrame, "UIPanelCloseButton")
    ...
    listFrame.hideIgnored = WHW_CreateFrame("CheckButton", "WordHunterWoWHideIgnored", listFrame, "UICheckButtonTemplate")
    ...
    listFrame.scroll = WHW_CreateFrame("ScrollFrame", nil, listFrame, "UIPanelScrollFrameTemplate")
    ...
    listFrame.content = WHW_CreateFrame("Frame", nil, listFrame.scroll)
