...
local function textHash(text)
  local hash = 5381
  for i = 1, WHW_len(text) do
    hash = math.mod(hash * 33 + string.byte(text, i), 4294967296)
  end
...
local function withoutPlayerName(text)
  ...
  return (string.gsub(text, (string.gsub(player, "(%W)", "%%%1")), "<name>"))
end
...
function Addon.HarvestText(kind, questId, text)
  ...
  if text == "" or WHW_len(text) > MAX_TEXT then return false end
...
function Addon.HarvestUnknownWord(word, questId)
  ...
  if word == "" or WHW_len(word) > 64 then return false end
  ...
  if not string.find(word, "%a") and not string.find(word, "[\192-\255]") then return false end
...
function Addon.rebuildHarvestExport()
  ...
  local bucket = corpusTable()
  local keys = {}
  for key in pairs(bucket) do keys[WHW_len(keys) + 1] = key end
  table.sort(keys)
  local rows = {}
  for _, key in ipairs(keys) do
    local entry = bucket[key]
    rows[WHW_len(rows) + 1] = table.concat({ entry.kind, tostring(entry.id or 0),
  ...
  if WHW_len(rows) == 0 then return 0 end
